...
function statusCopy(status?: PaymentAttemptStatus) {
  switch (status) {
    ...
    case 'PROCESSING':
    case 'PENDING':
      return 'Payment is still pending. WIEZ will keep checking with the backend.';
    case 'REQUIRES_ACTION':
      return 'Complete the secure provider step, then return here for backend verification.';
    default:
      return 'WIEZ is checking the latest backend payment state.';
  }
}
  ...
  const loadAttempt = useCallback(
    async (nextReference = reference) => {
      ...
      if (!normalizedReference) {
        ...
        setState('missing');
        setMessage('WIEZ could not find a payment reference to verify.');
        return null;
      }
        ...
        <ScrollView contentContainerStyle={styles.content}>
          <Card style={styles.card}>
            ...
            <AppText variant="body" tone="muted">
              WIEZ verifies this payment against the backend and provider before updating any order state.
            </AppText>
            {reference ? (
